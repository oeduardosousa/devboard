...
function Sidebar() {
  return (
    <>
      ...
      <nav className="flex w-full gap-2 overflow-x-auto rounded-2xl border border-white/10 bg-slate-950/50 p-2 lg:hidden">
        {navItems.map(({ to, label }) => (
          <NavLink
            key={to}
            to={to}
            className={({ isActive }) =>
              `flex min-w-fit flex-1 items-center justify-center rounded-xl px-4 py-3 text-sm font-medium transition ${
                isActive
                  ? "bg-linear-to-r from-violet-500/20 to-fuchsia-500/20 text-white ring-1 ring-violet-400/50"
